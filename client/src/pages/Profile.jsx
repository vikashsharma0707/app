...
function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        const res = await axios.get('https://app-2-rn6r.onrender.com/api/users/profile/' + JSON.parse(atob(token.split('.')[1])).id, {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await axios.put(`https://app-2-rn6r.onrender.com/api/users/${JSON.parse(atob(token.split('.')[1])).id}`, data, {
        headers: { 
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete your profile? This action cannot be undone.')) {
      const token = localStorage.getItem('token');
      try {
        await axios.delete(`https://app-2-rn6r.onrender.com/api/users/${JSON.parse(atob(token.split('.')[1])).id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
