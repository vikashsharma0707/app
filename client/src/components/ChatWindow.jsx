...
function ChatWindow({ selectedUser, currentUser }) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const res = await axios.get(`https://app-2-rn6r.onrender.com/api/users/messages/${selectedUser._id}`, {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const res = await axios.post('https://app-2-rn6r.onrender.com/api/users/messages', message, {
        headers: { Authorization: `Bearer ${token}` }
      });
